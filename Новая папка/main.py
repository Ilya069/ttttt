...
async def start(message: types.Message):
	...
	if result[0] == 0:
		if message.chat.id == admin:
			await message.answer('Добро пожаловать, @slivmenss.', reply_markup=kb.menu)
		else:
			await message.answer('Приветствую @slivmenss, это бот-саппорт.\nНапиши мне свой вопрос и я отправлю его администрации.\nЗа спам/флуд - ЧС!')
	else:
...
async def h(message: types.Message, state: FSMContext):
	...
	if result[0] == 0:
		...
		await message.answer('@slivmenss ты получил пизду в боте еблан.')
...
async def proce(message: types.Message, state: FSMContext):
	if message.text == '⏪ Отмена':
		...
		if message.text.isdigit():
			...
			await message.answer('Эйй @slivmenss!,Ты вводишь буквы...\nВведи ID')
...
async def proc(message: types.Message, state: FSMContext):
	if message.text == '⏪ Отмена':
		...
		if message.text.isdigit():
			...
			await message.answer('@slivmenss,Ты вводишь буквы...\nВведи ID')
